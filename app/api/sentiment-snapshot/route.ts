...
function parseCboePCR(pcCsv: string): { total?: number; equity?: number; index?: number } | null {
  ...
  out.total =
    pick(["total", "all"]) ??
    pick(["total put/call ratio"]) ??
    pick(["put/call ratio total"]);
...
const CBOE_CSV_CANDIDATES = [
  "https://cdn.cboe.com/data/us/equity/pc-ratio.csv", // new multi-column
];
const CBOE_LEGACY = [
...
function yahooHistoryUrl(symbol: string) {
  ...
  return `https://query1.finance.yahoo.com/v7/finance/download/${enc}?period1=0&period2=${nowSec}&interval=1d&events=history&includeAdjustedClose=true`;
}

async function getPutCall(): Promise<number | null> {
  // 0) Yahoo equity ratio first (fast and reliable): ^CPCE
  try {
    const csv = await fetchText(yahooHistoryUrl("^CPCE"));
    const n = parseYahooLatestClose(csv);
    if (Number.isFinite(n) && n! > 0 && n! < 5) return n!;
  } catch {}

  // 0b) Yahoo total ratio fallback: ^CPC
  try {
    const csv = await fetchText(yahooHistoryUrl("^CPC"));
    const n = parseYahooLatestClose(csv);
    if (Number.isFinite(n) && n! > 0 && n! < 5) return n!;
  } catch {}

  // 1) Newer CBOE multi-column
...
async function getVIX(): Promise<number | null> {
  ...
  try {
    const csv = await fetchText(yahooHistoryUrl("^VIX"));
...
export async function GET() {
  const sources: string[] = [
    ...
    "Yahoo ^CPCE/^CPC fallback + CBOE daily put/call CSV (pc-ratio.csv) + legacy totalpc.csv/equitypc.csv",
    "AAII (public CSV)",
  ];

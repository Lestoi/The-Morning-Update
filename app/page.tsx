...
type Sentiment = { fearGreed?: number; pcrTotal?: number; vix?: number; aaiiBulls?: number; aaiiBears?: number; note?: string; };
...
const flag = (iso2: string) => String.fromCodePoint(...iso2.toUpperCase().split("").map(c => 127397 + c.charCodeAt(0)));
...
function optionsExplanations(o?: OptionsBrief): string[] {
  ...
  bullets.push("Rule of thumb: bigger put tilt → faster first move but more mean-revert risk; lighter tilt → cleaner trends if the macro surprise is decisive.");
  return bullets;
}
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setNews(n.items ?? []); setYday(y.items ?? []);
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100 px-6 py-6">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        {/* Sentiment: each metric on its own line with hint */}
        <Card>
          <CardContent>
        ...
        {/* Options with bullets on fresh lines */}
        <Card>
          <CardContent>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
                    ...
                    {earnings.map((er, i) => (
                      <tr key={i} className="hover:bg-neutral-900/50">
                        ...
                        <td className="px-4 py-3">
                          <span className={`rounded-md px-2 py-1 text-xs ${er.session === "BMO" ? "bg-sky-600 text-white" : "bg-emerald-600 text-white"}`}>{er.session}</span>
                        </td>
          ...
          <Card>
            <CardContent>
              <h2 className="mb-3 text-lg font-semibold">Yesterday’s notable earnings (US)</h2>
              <div className="overflow-auto rounded-xl border border-neutral-800">
                <table className="w-full text-[15px]">
                  <thead className="bg-neutral-900/80">
                    <tr className="[&>th]:px-4 [&>th]:py-3 text-left text-neutral-300">
                    ...
                    {yday.map((r,i)=>(
                      <tr key={i} className="hover:bg-neutral-900/50">
                        ...
                        <td className="px-4 py-3 text-right">
                          <span className={`rounded-md px-2 py-1 text-sm font-semibold ${r.epsSurprisePct>=0 ? "bg-emerald-700 text-white" : "bg-rose-700 text-white"}`}>
                            {r.epsSurprisePct>=0?"+":""}{r.epsSurprisePct.toFixed(1)}%
                          </span>
                        </td>
                      </tr>
                    ))}
        ...
        <p className="text-[11px] text-neutral-500">Black theme enforced; sentiment & options on separate lines; yesterday’s earnings with color-coded beats/misses.</p>
      </div>
    </div>
  );
}
